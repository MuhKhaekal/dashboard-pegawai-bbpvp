    ...
    <main className="min-h-screen bg-[#f5f7fb] text-slate-800 overflow-x-hidden">
      ...
      <div className="relative max-w-[1600px] mx-auto px-4 md:px-6 lg:px-10 py-6 lg:py-8">
        ...
        <section className="grid grid-cols-2 xl:grid-cols-5 gap-4 mb-6">
          ...
          <KpiCard title="PNS" value={totalPNS} subtitle={`${persen(totalPNS)}% dari total`} icon="🛡️" gradient="from-emerald-500 to-teal-600" />

          <KpiCard title="PPPK" value={totalPPPK} subtitle={`${persen(totalPPPK)}% dari total`} icon="📋" gradient="from-blue-500 to-indigo-600" />

          <KpiCard title="Non ASN" value={totalNonASN} subtitle={`${persen(totalNonASN)}% dari total`} icon="👤" gradient="from-amber-400 to-orange-500" />

          <KpiCard title="ASN" value={totalASN} subtitle={`${persen(totalASN)}% dari total`} icon="🏛️" gradient="from-violet-500 to-purple-600" />
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <InsightCard type="danger" title="Perhatian Pensiun" value={`${retirementRisk} pegawai`} description="Memasuki masa pensiun tahun ini atau maksimal 2 tahun lagi." />

          <InsightCard type="warning" title="Cuti Kritis" value={`${criticalLeave} pegawai`} description="Memiliki sisa cuti maksimal 3 hari." />
          ...
          <div className="dashboard-card bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="grid grid-cols-3 gap-2 mt-6">
              {satpelStats.map((item) => (
                <div key={item.nama} className="rounded-2xl bg-slate-50 p-3 text-center hover:bg-orange-50 transition">
          ...
          <div className="lg:col-span-2 dashboard-card bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
              {Object.entries(generasi).map(([label, value]) => (
                <div key={label} className="group rounded-2xl p-4 bg-slate-50 hover:bg-indigo-50 transition-all">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
              <LeaveBox label="Kritis" value={cuti.kritis} color="red" />

              <LeaveBox label="Menipis" value={cuti.menipis} color="orange" />

              <LeaveBox label="Aman" value={cuti.aman} color="emerald" />

              <LeaveBox label="Berlebih" value={cuti.berlebih} color="sky" />
            </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="space-y-4 mt-6">
              {Object.entries(usia).map(([label, value]) => (
                <ProgressRow key={label} label={label} value={value} total={totalPegawai} color="bg-teal-500" />
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="flex gap-4 mt-8">
              ...
              <GenderCard label="Perempuan" value={gender.perempuan} percentage={totalASN ? Math.round((gender.perempuan / totalASN) * 100) : 0} icon="♀" color="pink" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="space-y-4 mt-6">
              {Object.entries(pensiun).map(([label, value]) => (
                <ProgressRow key={label} label={label} value={value} total={Math.max(...Object.values(pensiun), 1)} color={label === "Tahun Ini" ? "bg-red-500" : label === "1–2 Tahun" ? "bg-orange-500" : "bg-slate-400"} />
              ))}
            </div>
          </div>
        </section>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="space-y-5 mt-7">
              {Object.entries(pendidikan).map(([label, value]) => (
                <ProgressRow key={label} label={label} value={value} total={Math.max(...Object.values(pendidikan), 1)} color="bg-cyan-500" />
              ))}
            </div>
          </div>
        </section>
                    ...
                    {KATEGORI.map((kategori) => {
                      ...
                      return (
                        <td key={kategori.key} className="text-center px-3 py-4">
                          {value > 0 ? (
                            <span className="inline-flex w-8 h-8 items-center justify-center rounded-lg bg-violet-100 text-violet-700 font-black text-xs hover:scale-125 transition">{value}</span>
                          ) : (
                ...
                {PELAKSANA.map((jabatan, index) => {
                  ...
                  return (
                    <tr key={jabatan} className="border-b border-slate-100 hover:bg-orange-50/50 transition">
                      ...
                      <td className="text-center font-black text-[#15406A]">{item.total}</td>

                      <td className="text-center">{item.satpel > 0 ? <span className="px-3 py-1.5 rounded-lg bg-orange-100 text-orange-700 font-black text-xs">{item.satpel}</span> : "-"}</td>

                      <td className="text-center">{item.pusat > 0 ? <span className="px-3 py-1.5 rounded-lg bg-blue-100 text-blue-700 font-black text-xs">{item.pusat}</span> : "-"}</td>
                    </tr>
                  );
                })}
              </tbody>
...
function KpiCard({ title, value, subtitle, icon, gradient }: { title: string; value: number; subtitle: string; icon: string; gradient: string }) {
  return (
    <div className={`group relative overflow-hidden rounded-3xl bg-gradient-to-br ${gradient} text-white p-5 shadow-lg hover:-translate-y-1 hover:shadow-2xl transition-all duration-300`}>
...
function InsightCard({ type, title, value, description }: { type: "danger" | "warning" | "success"; title: string; value: string; description: string }) {
  const config = {
    danger: {
      bg: "bg-red-50",
  ...
  return (
    <div className={`rounded-3xl ${config.bg} ${config.border} border p-5 flex gap-4 hover:-translate-y-1 transition-all`}>
...
function LeaveBox({ label, value, color }: { label: string; value: number; color: string }) {
  ...
  return (
    <div className={`rounded-2xl border p-4 ${colors[color]} hover:-translate-y-1 transition-all`}>
...
function GenderCard({ label, value, percentage, icon, color }: { label: string; value: number; percentage: number; icon: string; color: "blue" | "pink" }) {
  const style =
    color === "blue"
      ? {
          bg: "bg-blue-50",
  ...
  return (
    <div className={`flex-1 rounded-2xl ${style.bg} p-4 text-center group hover:scale-[1.03] transition`}>
